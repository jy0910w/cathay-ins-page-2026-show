		...
		if ($(this).parents('.pop-menu').length&& this.name != 'document-menu') {
			$(this).on('select2:select',function(e) {
				...
				if ($(this).hasClass('filter')) {
                    ...
                    //憿舐內�凒憭� web/mobile�賊����蒀Start by Eva
                    functionlist.reset();
...
var checkIsScrolling_int;
var checkIsScrolling_direction;
var checkIsScrolling_status;

function checkIsScrolling(){

	var diff = $(window).scrollTop() - lastScrollPos;
	if(diff==0){
		$('html').removeClass('scrolling');
	}
	if($(window).scrollTop()<50){
		checkIsScrolling_status = false;
		$('.subnav').removeClass('slide-in');
		return;
	}else{
		if($(window).scrollTop() > lastScrollPos){
			if(checkIsScrolling_direction != 'down'){
				checkIsScrolling_direction = 'down';
				checkIsScrolling_status = false;
				clearTimeout(checkIsScrolling_int);
			}
		}else if($(window).scrollTop() < lastScrollPos){
			if(checkIsScrolling_direction != 'up'){
				checkIsScrolling_direction = 'up';
				checkIsScrolling_status = false;
				clearTimeout(checkIsScrolling_int);
			}
		}else{
	...
	switch(checkIsScrolling_direction){
		case 'up':
			$('.subnav').removeClass('slide-in');
		break;
		case 'down':
...
$(window).scroll(function () {
	...
	checkIsScrolling_int = setTimeout(checkIsScrolling,100);
...
///share 
function shareToFacebook(a_url){
	var url;
	if(a_url==undefined){
...
$(function(){
	$('[share-to-fb]').bind('click',function(){
		shareToFacebook();
	})
	$('[share-to-line]').bind('click',function(){
		shareToLine();
	})
	$('[copy-url]').bind('click',function(){
		$('#paste-box').remove();
		$('<textarea id="paste-box" style="position:absolute;top:-500px;"></textarea>').appendTo($('body'));
